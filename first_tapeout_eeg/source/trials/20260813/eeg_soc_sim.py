...
class SimConfig:
    ...
    frame_bits: int = 24  # 24-bit conversion payload
...
def quantize_to_signed_code(x: np.ndarray, full_scale_v: float, bits: int) -> np.ndarray:
    max_code = (1 << (bits - 1)) - 1
    min_code = -(1 << (bits - 1))
    scale = float(max_code)
    return np.clip(np.round((x / full_scale_v) * scale), min_code, max_code).astype(np.int32)
...
def save_block_io_numeric_csv(
    ...
    code_unit = f"LSB(int{cfg.adc_bits})"
...
def save_block_plot_png(
    ...
    axes[5].set_title(f"SD-ADC + decimation output ({adc_bits}-bit code)")
...
def simulate(cfg: SimConfig) -> None:
    ...
    if cfg.adc_bits < 2:
        raise ValueError("adc_bits must be >= 2")
    ...
    adc_code = quantize_to_signed_code(adc_out, cfg.adc_fs_v, cfg.adc_bits)
    ...
    print(f"ADC code width={cfg.adc_bits} bit")
    ...
    np.savetxt(
        ...
        delimiter=",",
        header=f"time_s,clean_after_pga_uV,ideal_decimated_uV,adc_out_uV,error_uV,fifo_spi_code_int{cfg.adc_bits}",
        comments="",
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--adc-bits", type=int, default=24, help="ADC output code width [bits]")
