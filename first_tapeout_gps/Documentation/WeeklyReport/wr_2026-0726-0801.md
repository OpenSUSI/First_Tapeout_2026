...
q_{mixed} = \sin\left(\omega_st\right)\sin\left(\omega_ft\right) \\
\end{align}
$$
...
$$
\begin{align}
i_{mixed} = \frac{1}{2} \lbrace \cos\left(\omega_st + \omega_ft\right) + \cos\left(\omega_st - \omega_ft \right) \rbrace   \\
q_{mixed} = \frac{1}{2} \lbrace \sin\left(\omega_st + \omega_ft\right) + \sin\left(\omega_st - \omega_ft \right) \rbrace \\
\end{align}
$$

となり、よく見ると上方に変換される成分と下方に変換される成分の２種類が生じており、ドップラーシフトを完全にはキャンセルできていないことがわかる。進んでしまった位相をもとに戻す操作が必要となるため、正しくは回転変換となるような操作が必要になる。

I/Q信号、ドップラー補正信号ともに直交する信号なので、複素数を使うと回転を簡単に考えることができる。

複素信号であるということは、受信信号は $I+jQ$ 、ドップラー補正信号は $\cos\left(-\omega_ft\right)+j\sin\left(-\omega_ft\right) = \cos\left(\omega_ft\right)-j\sin\left(\omega_ft\right)$ と表される。これをかけ合わせると、

$$
\begin{align}
\left( I+jQ \right) \left( \cos \left( \omega_ft \right) - j\sin \left( \omega_ft \right) \right) &= I \cos \left( \omega_ft \right) - Ij \sin \left( \omega_ft \right) + Qj \cos \left( \omega_ft \right) + Q \sin \left( \omega_ft \right) \\
& = \lbrace I\cos\left(\omega_ft\right) + Q\sin\left(\omega_ft\right) \rbrace + j\lbrace Q \cos\left( \omega_ft \right) - I\sin \left( \omega_ft \right) \rbrace
\end{align}
$$
...
\left\\{
\begin{align}
i_{mixed} &= i\cos\left(\omega_ft\right) + q\sin\left(\omega_ft\right) \\
q_{mixed} &= q\cos\left(\omega_ft\right) - i\sin\left(\omega_ft\right)
\end{align}
\right.
$$
...
# ドップラーシフトの補正
i_mixed = carrier_i * i + carrier_q * q
q_mixed = carrier_i * q - carrier_q * i
```

この処理の変更により相関がS/Nが良くなることが確認でき、従来の処理が誤っていたことも確認した。修正前後のコード探索結果を下図に示す。corrの数値が5119から6015まで改善していることがわかる。

![rotation_ack](figure/2026-0726-0801/rotation.png)
...
ここでコード遅延が受信信号とズレていたときの相関値を考えると、下の図のようになる。
...
このようにして求めた角速度をドップラー周波数の推計値として使用して、次のサンプルのドップラーシフト補正を行う。
...
信号の位相をある値にロックする制御がPLL(Phase locked loop)である。FLLまで行うとドップラー周波数が一致した状態になるので、FLLで周波数引き込み(PULL IN)を行い、十分に周波数が一致した状態で制御をPLLに移行して、ドップラーの位相もロックすることでIの相関を見ることで、航法データを復調できるようになる。

来週はPLLを実装し、航法データの復調までを実装する。
